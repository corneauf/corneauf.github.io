...
<!--more-->
See, this languages supports two fundamental types: strings and floats. 1? that's a float. 2.0? That's a float. Therefore, when parsing the code, you need to convert the string representation "1.0" into a float. So, there you are, coding away and you need to parse a string to a float, what is your first instinct? Yes, look at cppreference.com. Let us see what we have:
